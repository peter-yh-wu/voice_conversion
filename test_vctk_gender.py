...
if __name__ == '__main__':
    ...
    if args.tag == '':
        new_wav_dir = os.path.join(data_dir, 'wav_gvc_gender')
    else:
        new_wav_dir = os.path.join(data_dir, 'wav_gvc_gender_%s' % args.tag)
    ...
    with torch.no_grad():
        for path in tqdm(all_wav_paths):
            ...
            if not os.path.exists(output_path):
                ...
                c = torch.from_numpy(np.array([int(args.target)])).cuda(0)
                if spec_tensor.shape[1] > 8:
